...
class Model(nn.Module):
    def __init__(self, sizes, dis_sim, mic_sim):
        ...
        self.gcn_1 = conv.GCNConv(self.dis_size + self.mic_size, self.F1)
        # ---------------------------------------
        # self.dgi = DGI(self.dis_size + self.mic_size, self.F1, 'prelu')
        # self.b_xent = nn.BCEWithLogitsLoss()
        # ---------------------------------------
        self.gcn_2 = conv.GCNConv(self.F1, self.F2)
        self.gcn_3 = conv.GCNConv(self.F2, self.F3)

        self.alpha1 = t.randn(self.dis_size, self.mic_size).double()
        self.alpha2 = t.randn(self.mic_size, self.dis_size).double()
    ...
    def forward(self, input):
        ...
        # -------------------------------------------
        # idx = np.random.permutation(self.dis_size + self.mic_size)
        # shuf_fts = x[idx, :]
        # lbl_1 = t.ones(1, self.dis_size + self.mic_size)
        # lbl_2 = t.zeros(1, self.dis_size + self.mic_size)
        ...
        diss_kernels.append(t.DoubleTensor(getGipKernel(H1[:self.dis_size].clone(), 0, self.h1_gamma, True).double()))
        mic_kernels.append(t.DoubleTensor(getGipKernel(H1[self.dis_size:].clone(), 0, self.h1_gamma, True).double()))

        H2 = t.relu(self.gcn_2(H1, adj['edge_index'], adj['data'][adj['edge_index'][0], adj['edge_index'][1]]))
        diss_kernels.append(t.DoubleTensor(getGipKernel(H2[:self.dis_size].clone(), 0, self.h2_gamma, True).double()))
        mic_kernels.append(t.DoubleTensor(getGipKernel(H2[self.dis_size:].clone(), 0, self.h2_gamma, True).double()))

        H3 = t.relu(self.gcn_3(H2, adj['edge_index'], adj['data'][adj['edge_index'][0], adj['edge_index'][1]]))
        diss_kernels.append(t.DoubleTensor(getGipKernel(H3[:self.dis_size].clone(), 0, self.h3_gamma, True).double()))
        mic_kernels.append(t.DoubleTensor(getGipKernel(H3[self.dis_size:].clone(), 0, self.h3_gamma, True).double()))

        diss_kernels.append(self.dis_sim)
        mic_kernels.append(self.mic_sim)

        dis_k = sum([self.dis_ps[i] * diss_kernels[i] for i in range(len(self.dis_ps))])
        ...
        # -------------------------------------------
        con_loss_1 = self.SSL(dis_k, self.dis_sim)
        con_loss_2 = self.SSL(mic_k, self.mic_sim)
        # -------------------------------------------

        out1 = t.mm(self.dis_k, self.alpha1)
